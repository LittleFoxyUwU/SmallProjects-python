...
def password_rules_input():
    ...
    if input('Will your password/s will have UPPERCASE LETTERS input yes to include them, or anything else to exclude!\n').lower() in 'yes':
        have_uppercase = True
    else:
        have_uppercase = False

    if input('Will your password/s will have lowercase letters? input yes to include them, or anything else to exclude!\n').lower() in 'yes':
        have_lowercase = True
    else:
        have_lowercase = False

    if input('Will your password/s will have symbols (!#$%&*+-=?@^_.)? Input yes to include them, or anything else to exclude!\n').lower() in 'yes':
        have_symbols = True
    else:
        have_symbols = False

    if input('Will your password/s will have bad symbols (il1Lo0O)? Input yes to include them, or anything else to exclude!\n').lower() in 'yes':
    ...
    if input('Do you agree with those attributes? Print yes to start generating, or anything else if you want to re-enter them!\n').lower() in 'yes':
